...
def get_reader_func(name):
    ...
    elif name == "pgm_composite_to_dataset":
        return pgm_composite_to_dataset
    else:
...
def set_time_and_swap_xy(xda):
    xda = xda.assign_coords(dict(time=[datetime.strptime(str(xda.time.values[0].astype(int)), "%Y%m%d%H%M")]))
...
def metranet_to_dataset(fn, var_name=None):
    ...
    if quantity == "NWP_HZEROCL":
        timestamp = datetime.strptime(re.findall(r"\d+", fn.name.split(".")[0])[0], "%y%j%H%M0")
    else:
        timestamp = datetime.strptime(re.findall(r"\d+", fn.name.split(".")[0])[0], "%y%j%H%M")
    ...
    if "ECHOTOP" in quantity:
        ...
        # Vetically integrated liquid
        ds_attrs = {"long_name": "Vertically integrated liquid", "units": "kg/m^2"}
        if var_name is None:
            var_name = "VIL"
    elif "Column_Max" in quantity:
        # Max echo
        ds_attrs = {"long_name": "Column max reflectivity", "units": "dBZ"}
        if var_name is None:
            var_name = "MAX_ECHO"
    else:
...
def pgm_composite_to_dataset(fn, var_name=None, **kwargs):
    """
    Read FMI PGM gzip composite file and return as xarray dataset.
    ...
    fn : str
        File name.
    var_name : str, optional
        Variable name used, by default None
    **kwargs: dict
        Additional keyword arguments. In order to transform the data to rain rate, set
        `transform_to_rate` to True. If this is done, the following additional arguments
        are required:
            - `zr_a`: The Z-R relationship coefficient a.
            - `zr_b`: The Z-R relationship coefficient b.
        To set the projection, set `proj4` to the proj4 string of the projection, and
        `LL_lon`, `LL_lat`, `UR_lon`, `UR_lat` to the corner points of the projection.
    ...
    """
    data = np.flipud(imread(gzip.open(fn, "r")))
    mask = data == 255
    data = data.astype(np.float64)
    data = (data - 64.0) / 2.0
    data[mask] = np.nan

    if kwargs.get("transform_to_rate", False):
        lin_z = 10 ** (data / 10.0)

        zr_a = kwargs.get("zr_a", 223)
        zr_b = kwargs.get("zr_b", 1.53)

        data = (lin_z / zr_a) ** (1 / zr_b)
    ...
    # read pgm file header and read active radars used in composite
    active_radars = {}
    with gzip.open(fn, "r") as f:
        ...
        while line[0] == "#":
            ...
            words = line.split()
            if "radar" in words:
                active_radars[words[2]] = words[3]

    # Get projection
    if kwargs.get("proj4", None) is not None:
        crs = CRS.from_proj4(kwargs["proj4"])
    ...
    if kwargs.get("transform_to_rate", False):
        ...
        ds_attrs = {"long_name": "Reflectivity", "units": "dBZ", "fileprefix": "z"}
...
def load_data(ds_conf: dict, curdate: datetime, obstimes: list, num_obs: int, radar: str):
    ...
    for name in ds_conf.keys():
        # Read data
        pathformat = str(Path(ds_conf[name].path.format(radar=radar)) / Path(ds_conf[name].file.format(radar=radar)))
        ...
        filetimes = pd.DataFrame(
            {
                "time": [
                    datetime.strptime(p.name.split(".")[0], filename_pattern.split(".")[0]).replace(second=0)
        ...
        if ds_conf[name].file_timestamp == "exact":
            ...
            try:
                ...
                raise FileNotFoundError(f"No files found for {name} at {rootpath}!") from e
            ...
            try:
                files = filetimes.iloc[(filetimes["time"] - curdate).abs().argsort().iloc[:num_obs]].filename.to_list()[
                    ::-1
                ]
            ...
            raise ValueError(f"Unknown file_timestamp method {ds_conf[name].file_timestamp} for{name}!")
        ...
        try:
            if isinstance(ds_conf[name].get("reader_func"), str):
                reader_func = get_reader_func(ds_conf[name]["reader_func"])
                reader_args = ds_conf[name].get("reader_args", {})
                ds_ = [reader_func(f, var_name=var_name, **reader_args) for f in files]
                datasets[name] = xr.concat(ds_, dim="time")
            else:
                if isinstance(ds_conf[name].get("preprocess_func"), str):
        ...
        if isinstance(ds_conf[name].get("theoretical_max"), float):
            datasets[name][var_name] = datasets[name][var_name].where(
                datasets[name][var_name] < ds_conf[name]["theoretical_max"]
            )
        if isinstance(ds_conf[name].get("theoretical_min"), float):
            datasets[name][var_name] = datasets[name][var_name].where(
        ...
        datasets[name][var_name].attrs[f"input_files"] = [str(f.resolve()) for f in files]

    dataset = xr.merge([d for _, d in datasets.items()], compat="override", join="override")

    # Drop z dimension for now if it exists
    if "z" in dataset.sizes.keys():
        ...
        filetimes = pd.DataFrame(
            {
                ...
                "filename": list(Path(day.strftime(rootpath_format)).glob(filename_glob)),
            }
        ).sort_values("datatime")

        if minute_divisible_by is not None:
            # Remove unwanted minutes
            filetimes = filetimes[filetimes["datatime"].dt.minute % minute_divisible_by == 0]
...
def compute_advection_field(
    ...
    images_filtered = _filtered_ubyte_images(np.stack([image1, image2]), filter_stddev, minval=minval)
